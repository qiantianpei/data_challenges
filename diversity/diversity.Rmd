...
### Step 3: simple exploratory analysis
...
employee_ranks_processed <- 
  ...
  summarize(x = unique(employee_id)) %>% count()
...
We will build different models to answer the question from different perspectives. 

#### Sign on bonus
Is sign on bonus given out fairly?

Note that in answering this question we shouldn't include more variables than needed!! This is a common pitfall that people new to modelling can fall into. For example, gender may affect the level the person gets and the level can subsequently decide whether the person gets sign on bonus. If you include "level" in your model when studying the impact of gender, then you will get an inaccurate estimate.
...
A quick plot shows that bonus rate seems irrelevant to sex and degree level but seems to be lower in the HR department. There is no obvious interaction pattern between these variables. 
...
bonus_lm %>% summary()
```
Indeed, each department has significantly different bonus rates, with the HR department being the lowest and the engineering department being the highest.
...
employee_final %>% 
  ...
  geom_line(aes(color = degree_level, group = degree_level))
```
The HR department clearly hires more females than males while the other 3 departments hire more males.
...
employee_final %>% 
  ...
  pivot_longer(cols = high_school:phd, names_to = "degree", values_to = "rate") %>% 
  ggplot() +
  geom_line(aes(dept, rate, color = degree, group = degree)) +
  facet_grid(cols = vars(sex))
```
On the other hand, all departments seem to have very similar hiring standards for degree levels.
...
Not surprisingly, you have roughly equal opportunities to become managers in all departments. Also, higher degrees and more years of experience increase the odds. Lastly and notably, females are less likely to become managers in this company!
...
salary_lm <- 
  lm(
    salary ~ degree_level + sex + dept + yrs_experience + rank + num_reports, 
    data = employee_final
  )
```
